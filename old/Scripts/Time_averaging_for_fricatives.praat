...
#Copyright 2013, Christian DiCanio, Haskins Laboratories. Special thanks to Christine Shadle for suggestions and troubleshooting.

form Extract Fricative data from labelled points
   sentence Directory_name: /Projects/Mixtec_consonants/Consonant_list_cuts/fricatives/
   sentence Interval_label fr
   sentence Log_file logfile_fricatives_15ms_6_300Hz
...
#a matrix. So, instead, you have to extract the first two moments and then multiply each by the sampling interval size.
				...
				skew = Get skewness... 2
				kurt = Get kurtosis... 2
				fileappend 'directory_name$''log_file$'.txt 'd1''tab$''intensity''tab$''cog''tab$''sdev''tab$''skew''tab$''kurt''newline$'
			else
				#do nothing
	   		endif
	endfor
